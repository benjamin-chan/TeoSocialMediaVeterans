...
df2 %>% group_by(image, text) %>% summarize(n = length(record_id))
df2 %>% group_by(consent, eligible, consent_and_eligibility_complete) %>% summarize(n = length(record_id))
df2 %>% group_by(eligible, indSurveyComplete) %>% summarize(n = length(record_id))
df2 %>% group_by(eligible, indSurveyComplete, va_ever_enrolled, va_use_12mo) %>% summarize(n = length(record_id))
